<x-login-layout type="Teacher">
    <form class="space-y-4 md:space-y-6" method="POST" action="{{ route('ins.login.store') }}">
        @csrf
        <div class="form-group" x-data="{ isError: {{ $errors->has('mobile') ? 'true' : 'false' }} }">
            <label for="mobile" class="label">Your mobile</label>
            <input @click="isError = false" type="number" name="mobile" class="form-control"
                :class="{ 'is-invalid': isError }" placeholder="01********" value="01717464617" required="">
            @error('mobile')
        ...
        <div class="form-group" x-data="{ isError: {{ $errors->has('password') ? 'true' : 'false' }} }">
            ...
            <input type="password" name="password" id="password" placeholder="••••••••" class="form-control"
                :class="{ 'is-invalid': isError }" required="">
            @error('password')
                <div class="invalid-feedback" x-show="isError">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="flex justify-around">
            <a class="guerd-btn" href="{{ route('login') }}">Admin Login</a>
             
                <a class="guerd-btn" href="{{route('student.login')}}">Student Login</a>
        </div>
    </form>
</x-login-layout>

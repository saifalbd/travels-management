<x-login-layout type="Admin">
    <form class="space-y-4 md:space-y-6" method="POST" action="{{route('admin.login.store')}}">
        @csrf
        <div class="form-group" x-data="{ isError: {{$errors->has('email')?'true':'false'}} }">
            <label for="email" class="label">Your email</label>
            <input @click="isError = false" type="email" name="email" class="form-control" :class="{'is-invalid':isError}" placeholder="name@company.com" required="">
            @error('email')
